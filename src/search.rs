...
if chapter_entry.is_none() {
    println!("Chapter '{}' not found in book '{}'.\n{} has {} chapters.", chapter, kebob_to_title(book), kebob_to_title(book), book_entry.unwrap().chapters.len());
    return None;
}

let verse_entry = chapter_entry.unwrap().verses.get(verse);
if verse_entry.is_none() {
    println!("Verse '{}' not found in book '{}' chapter '{}'.\nChapter {} of {} has {} verses", verse, kebob_to_title(book), chapter, chapter, kebob_to_title(book), chapter_entry.unwrap().verses.len());
    return None;
}
